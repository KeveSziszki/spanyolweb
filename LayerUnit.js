class LayerUnit extends LayerBase {
    ...
    draw() {
        ...
        this.imageLoader.SubscribeAllLoaded(() => {
            this.imageLoader.Items.forEach((item) => {
    ...
    AttackWithUnit(unit, targetCell, targetUnit) {
    }
    ClearUnit(unit) {
        this.clearUnitInternal(this.ctx, unit);
    }
    clearUnitInternal(ctx, unit) {
    ...
    drawUnitAt(ctx, unit, point) {
        ...
        ctx.drawImage(img, 0, 0, img.width, img.height, point.x - size / 2, point.y - size / 2, size, size);
        if (unit.IsHooman) {
            ctx.beginPath();
            ctx.fillStyle = "green";
