	class GameController
	{
		...
		public layerUnit: LayerUnit;
		public layerTerrain: LayerTerrain;
		private input: InputHandler | undefined;
		public Container: HTMLElement;
		public Window: Rect;
		public BoardHeight: number;
		public BoardWidth: number;
		public Scale: number = 1;
		public PlayerUnits: GameUnit[];
		public AIUnits: GameUnit[];
		public CurrentTurn:number = 0;
		private maxTurn: number = 15;
		private cities: Pont[]
		private remCities: number = 0
		public allowInput: boolean = true;
		public UnitStats?: Record<UnitType,UnitStat>;
		...
		constructor() {
			...
			this.UnitStats = {
			cavalry : {HP: 12, AP: 5, Range: 1,Movement: 2},
			foot : {HP: 10, AP: 4, Range: 1,Movement: 1},
			siege : {HP: 5, AP: 15, Range: 1,Movement: 1}
			};
		}
		...
		public start=()=>
		{
			...
			for (var row = 0; row < this.Rows; row++) {
				let currentRow: GameCell[] = [];
				for (var col = 0; col < this.Cols; col++) {
					let center = this.GridMan.GridToCenter(col, row);
					currentRow.push(new GameCell(row, col,
						row * this.Cols + col,
						center, row == (this.Rows - 1)));
				}
			...
			for (var i = 0; i < this.cities.length; i++) {
				...
				foot.Cell = cellCity; cellCity.Unit = foot;
			}
		...
		//numberek padding
		private randomizeUnits(lineFrom:number, lineTo:number, padLeft:number, padRight:number, units:GameUnit[]): void {
		...
		public NextTurn()
		{
			...
			if (this.CurrentTurn <= this.maxTurn)
			{
				...
				this.cities.forEach((c) => {
					if (this.Cells[c.y][c.x].Type == TerrainType.City && this.Cells[c.y][c.x].Unit != undefined && this.Cells[c.y][c.x].Unit?.IsHooman)
					{
		...
		public PurchaseUnit(type: UnitType,cell: GameCell)
		{
			let stat = this.UnitStats[type];
			let newUnit = new GameUnit(type, stat.HP, stat.AP, stat.Range, stat.Movement, true)
			newUnit.TurnMovementLeft = 0;
			newUnit.CanAttack = false;
			this.PlayerUnits.push(newUnit);
			newUnit.Cell = cell;
			cell.Unit = newUnit;
			this.layerUnit.DrawUnitPublic(newUnit,cell)
		}

		public MoneyRedraw()
		{
			let MoneyHtml = document.getElementById("money")
			MoneyHtml!.textContent = `Money: ${this.Money}ł`
		}
